...
#>
    <#
        .SYNOPSIS
            Create a Message Box for NOTIFICATIONS
...
function GetScriptImagesPath{       # NOEXPORT
    try {
        $script:MyPicturesFolderPath = [Environment]::GetFolderPath("MyPictures")
    }
    catch {
        $script:MyPicturesFolderPath = $null
    }

    $ImagesPath = ''

    $script:MyDocumentsPSPath = if ($script:MyPicturesFolderPath) {
        $ImagesPath = Join-Path -Path $script:MyPicturesFolderPath -ChildPath "ScriptImages"
    }
    else {
        $ImagesPath = 'c:\' 
        $ImagesPath += Join-Path -Path $env:USERPROFILE -ChildPath "Pictures\ScriptImages"
    }
    return $ImagesPath
}


function InvokeMissionImpossible {      # NOEXPORT
...
$Script:WindowsAssemblyReferences = [System.Collections.ArrayList]::new()
$Script:WindowsAssemblyReferences += 'System' | Out-Null
$Script:WindowsAssemblyReferences += 'System.Drawing' | Out-Null
$Script:WindowsAssemblyReferences += 'System.Xml' | Out-Null
$Script:WindowsAssemblyReferences += 'System.Windows.Forms' | Out-Null
$Script:WindowsAssemblyReferences += 'PresentationFramework' | Out-Null
$Script:WindowsAssemblyReferences += 'PresentationCore'   | Out-Null
...
$Source = Join-Path (GetScriptImagesPath) 'Notif.png'
...
$export_MissionImpossible = [scriptblock]::Create(@"
  Function MissionImpossible { $function:InvokeMissionImpossible }
"@)

if($NoSound -eq $False){
    Start-Job -ScriptBlock {MissionImpossible} -InitializationScript $export_MissionImpossible | Receive-Job
    
}

try{
    Show-MessageBox -Content $StackPanel -Title "$MsgBoxTitle" -TitleBackground $Color -TitleTextForeground Black -ContentBackground $Color -TitleFontWeight UltraBold 
}catch{
