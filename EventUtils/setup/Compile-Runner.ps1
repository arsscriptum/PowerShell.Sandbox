...
try{
    ...
    Write-ChannelMessage  "Compile-ClearEventEntries"
    Write-ChannelMessage  "====================================="
    ...
    $RunnerScriptPath = Join-Path $SetupPath 'ClearEventEntries.ps1'
    $ImagePath = Join-Path $ImgPath 'MyShim.png'
    $IconPath = Join-Path $ImgPath 'Manga-red.ico'
    $RunnerPath = Join-Path $BinPath 'ClearEventEntries.exe'
    ...
    Invoke-ps2exe -inputFile $RunnerScriptPath -outputFile "$RunnerPath" -iconFile "$IconPath" -noConsole -requireAdmin -noError -noOutput
